...
st.markdown(
    ...
    .ph-meta-item { display: flex; flex-direction: column; }
    .ph-meta-label {
        font-size: 0.6rem;
        color: #A0B4C8;
        font-weight: 600;
    ...
    .ph-stale { color: #C53030 !important; }
...
# ── Sidebar ───────────────────────────────────────────────────────────────────
with st.sidebar:
    # Language toggle — topmost
    st.markdown(
        f'<div class="sb-section">{S["lang_label"]}</div>',
        unsafe_allow_html=True,
    )
    lang_options = ["한국어", "English"]
    lang_index   = 0 if lang == "ko" else 1
    ...
    <div class="page-header">
        ...
        <div class="ph-meta">
            <div class="ph-meta-item">
                <span class="ph-meta-label">{html.escape(S["last_updated"])}</span>
                <span class="ph-meta-value">{html.escape(fetched_at_str)}</span>
            </div>
            <div class="ph-meta-item">
                <span class="ph-meta-label">{html.escape(S["latest_article"])}</span>
                <span class="ph-meta-value {stale_class}">{html.escape(latest_ist_str)}</span>
            </div>
            <div class="ph-meta-item">
                <span class="ph-meta-label">{html.escape(S["period"])}</span>
                <span class="ph-meta-value">{html.escape(selected_range_label)}</span>
            </div>
        </div>
    </div>
    """,
    ...
    if df_pool.empty:
        ...
        fig_bar.update_layout(
            **PLOTLY_LAYOUT,
            ...
            xaxis=dict(title=S["mentions_xaxis"], showgrid=True,
                       gridcolor="#EEF4FA", zeroline=False),
    ...
    if df_pool.empty:
        ...
        fig_sov.update_layout(
            **PLOTLY_LAYOUT,
            height=360,
            yaxis=dict(title="", tickfont=dict(size=11)),
            xaxis=dict(title=S["sov_xaxis"], showgrid=True,
                       gridcolor="#EEF4FA", zeroline=False),
            coloraxis_showscale=False,
        )
...
if df_filtered.empty:
    ...
    fig_signal.update_layout(
        **PLOTLY_LAYOUT,
        ...
        yaxis=dict(title=S["signal_yaxis"], showgrid=True,
                   gridcolor="#EEF4FA", zeroline=False),
        legend=dict(
...
if df_filtered.empty:
    ...
    disp = disp[[S["col_article"], "Brand", "Category", "Published"]].rename(columns={
        "Brand":     S["col_brand"],
        "Category":  S["col_type"],
...
with st.expander(S["dq_title"], expanded=False):
    ...
    if not df_brand.empty:
        ...
        if days_stale is None:
            ...
            f_cls, f_dot, f_msg = "st-bad",  "●", S["dq_stale_msg"].format(days=days_stale)
        else:
